...
export class ChatService {
  ...
  public async startChat() {
    ...
    // open WebSocket, any modification in our POD will cause the execution of "checkInbox()".
      ...
      if (channel != null) {
        ...
        chatChannel.participants.forEach(async participant => {
          if (participant.webId != this.webid) {
            ...
            // add reading permissions to the file for the participant
            await this.rdf.addReaderToACL(urlFile, participant.webId.toString());
            // send to the inbox of the participant the message with the url of the file
  ...
  private async processRegularMessage(newMessage: any): Promise<string> {
    ...
    if (channel != null) {
      ...
      // save the message in the chat on our own POD
